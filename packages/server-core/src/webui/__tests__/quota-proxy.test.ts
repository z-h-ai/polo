 ...
 * - Polo AI server exposes GET /api/quota/status (authenticated by Admin JWT)
 * - Server extracts userId from the request auth context, looks up stored JWT from in-memory map
...
describe('GET /api/quota/status', () => {
  it('returns 401 when no session cookie is provided', async () => {
    ...
    setEnv('POLO_ADMIN_API_URL', 'http://admin.example.com')
  ...
  it('returns 503 when POLO_ADMIN_API_URL is not set', async () => {
    setEnv('JWT_SECRET', JWT_SECRET)
