...
import { canonicalTransportHref, failureResult, serializeHostResult, validateHostRequest, writeHostResultLine, type HostCompletionResultV1, type HostFailureKind, type ValidatedHostRequest } from './host-completion-protocol.ts'
...
async function readRequestLine(): Promise<{ ok: true; line: string; firstByteAt: number } | { ok: false; oversize: boolean }> {
...
export async function runHostCompletion(argvInvalid = false): Promise<void> {
  let settled = false, activeRequestId = 'invalid', activeModel = 'invalid', timer: NodeJS.Timeout | null = null, installed: InstalledTransportObservation | null = null
  ...
  process.on('uncaughtException', () => failWith('provider_error_terminal'))
  process.on('unhandledRejection', () => failWith('provider_error_terminal'))
  if (argvInvalid) return failWith('invalid_worker_message')
  try {
    const read = await readRequestLine()
    ...
    const request = validateHostRequest(parsed)
    if (!request) return failWith('invalid_worker_message')
    ...
    const deadlineAt = read.firstByteAt + request.timeoutMs
    timer = setTimeout(() => failWith('deadline_exceeded'), Math.max(0, deadlineAt - Date.now()))
    sanitizeEnvironment()
    if (!checkDescriptor(request)) return failWith('invalid_worker_message')
    if (isBedrock && credential.type === 'iam') {
    ...
    // Fixed post-EOF order: policy module + AWS dep load first, seam install, then pi-ai imports.
    ...
    ]) as [typeof import('@mariozechner/pi-ai'), typeof import('@mariozechner/pi-ai/bedrock-provider'), typeof import('@mariozechner/pi-ai/oauth') | null]
    piAi.setBedrockProviderModule(bedrockProviderModule.bedrockProviderModule)
    const getCatalog = piAi.getModels as unknown as (provider: string) => Array<Record<string, any>>
    ...
    if (request.routeKind === 'custom') {
      ...
      const hit = getCatalog(request.provider).find((entry) => entry.id === bareId)
      if (!hit) return failWith('catalog_model_missing')
      model = { ...hit }
      ...
      if (copilotBaseUrl !== null) {
        if (canonicalOf(copilotBaseUrl) !== request.transportHref) return failWith('invalid_worker_message')
        model.baseUrl = copilotBaseUrl
      } else if (!isBedrock) {
