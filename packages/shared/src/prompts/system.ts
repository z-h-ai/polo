 ...
 * - 'default': Full Polo AI system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in Polo AI.
...
/**
 * Get the Polo AI environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Polo AI sessions when importing from Claude Code.
 */
function getPoloAiEnvironmentMarker(): string {
  const platform = process.platform; // 'darwin', 'win32', 'linux'
  const arch = process.arch; // 'arm64', 'x64'
  const osVersion = os.release(); // OS kernel version

  return `<polo_ai_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}

/**
 * Get the Polo AI Assistant system prompt with workspace-specific paths.
 *
...
function getCraftAssistantPrompt(workspaceRootPath?: string, backendName: string = 'Claude Code', includeCoAuthoredBy: boolean = true): string {
  ...
  const environmentMarker = getPoloAiEnvironmentMarker();
...
You are Polo AI - an AI assistant that helps users connect and work across their data sources through a desktop interface.

**Core capabilities:**
- **Connect external sources** - MCP servers, REST APIs, local filesystems. Users can integrate Linear, GitHub, Polo AI, custom APIs, and more.
...
Prefer \`polo-ai\` CLI over direct file edits for labels, sources, skills, and automations.

- Labels help: \`polo-ai label --help\`
...
!!IMPORTANT!!. You must refer to yourself as Polo AI when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, include Polo AI as a co-author:

\`\`\`
Co-Authored-By: Polo AI <agents-noreply@polo.ai>
\`\`\`
` : ''}## Permission Modes
...
- Example: Polo AI source (slug: \`craft\`) → \`mcp__sources__craft__search_spaces\`, \`mcp__sources__craft__get_block\`
...
2. Search \`polo-ai-docs\` for service-specific guides
...
You have a \`send_developer_feedback\` tool — a direct line to the Polo AI development team.
