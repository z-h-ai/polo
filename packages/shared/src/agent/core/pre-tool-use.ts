...
import {
  CLI_DOMAIN_POLICIES,
  POLO_AI_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES,
  POLO_AI_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES,
...
export function qualifySkillName(
  ...
  // Extract the bare slug — strip any existing qualifier (e.g. "PoloAiWS:commit" → "commit")
...
export function getConfigCliRedirect(
  ...
  if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
    ...
    if (relativePath) {
      const labelsScopeMatch = POLO_AI_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES.find(
...
/**
 * Block bash commands that operate on guarded config paths unless they use polo-ai commands.
...
export function getConfigDomainBashRedirect(
  ...
  if (/^polo-ai\s+(label|automation|source|skill)\b/.test(command)) {
    return null;
  }
  ...
  const bashGuardEntries: Array<{ namespace: CliDomainNamespace; scope: string }> = POLO_AI_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES
...
const BUILT_IN_MCP_SERVERS = new Set(['session', 'polo-ai-docs']);
...
export function runPreToolUseChecks(ctx: PreToolUseInput): PreToolUseCheckResult {
  ...
  // 5b. Config-domain Bash guard (block direct labels/automations path operations unless using polo-ai)
  if (FEATURE_FLAGS.poloAiCli && toolName === 'Bash') {
