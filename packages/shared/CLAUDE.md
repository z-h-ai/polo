...
- WebUI source OAuth uses the stable relay redirect URI (`https://app.polo.z-h-ai.com/auth/callback`). The public relay stores a short-lived, one-time opaque state before authorization and only redirects to a registered callback target.
