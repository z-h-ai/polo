 ...
 *   POLO_AI_RPC_TLS_KEY          — path to PEM private key file (required with cert)
 *   POLO_AI_RPC_TLS_CA           — path to PEM CA chain file (optional)
 *   POLO_AI_APP_ROOT             — app root path (default: cwd)
 *   POLO_AI_RESOURCES_PATH       — resources path (default: cwd/resources)
...
import type { WsRpcTlsOptions } from '@polo-ai/server-core/transport'
...
import { initModelRefreshService, setFetcherPlatform } from '@polo-ai/server-core/model-fetchers'
...
if (process.env.POLO_AI_DEBUG === 'true' || process.env.POLO_AI_DEBUG === '1') {
  enableDebug()
}
...
// In packaged mode, use POLO_AI_BUNDLED_ASSETS_ROOT env or cwd.
const bundledAssetsRoot = process.env.POLO_AI_BUNDLED_ASSETS_ROOT
  ?? join(import.meta.dir, '..', '..', '..', '..')

// TLS configuration — when cert + key paths are provided, server listens on wss://
let tls: WsRpcTlsOptions | undefined
const tlsCertPath = process.env.POLO_AI_RPC_TLS_CERT
const tlsKeyPath = process.env.POLO_AI_RPC_TLS_KEY
if (tlsCertPath || tlsKeyPath) {
  if (!tlsCertPath || !tlsKeyPath) {
    console.error('TLS requires both POLO_AI_RPC_TLS_CERT and POLO_AI_RPC_TLS_KEY.')
    process.exit(1)
  }
  tls = {
    cert: readFileSync(tlsCertPath),
    key: readFileSync(tlsKeyPath),
    ...(process.env.POLO_AI_RPC_TLS_CA ? { ca: readFileSync(process.env.POLO_AI_RPC_TLS_CA) } : {}),
  }
}
...
const waWorkerEntry = process.env.POLO_AI_MESSAGING_WA_WORKER
  ?? join(bundledAssetsRoot, 'packages', 'messaging-whatsapp-worker', 'dist', 'worker.cjs')
const waNodeBin = process.env.POLO_AI_MESSAGING_NODE_BIN ?? 'node'
...
const healthPort = parseInt(process.env.POLO_AI_HEALTH_PORT ?? '0', 10)
...
if (!isLocalBind && instance.protocol === 'ws') {
  if (process.argv.includes('--allow-insecure-bind')) {
    console.warn(
      ...
      '   Set POLO_AI_RPC_TLS_CERT and POLO_AI_RPC_TLS_KEY to enable wss://.\n'
    )
  } else {
    console.error(
      ...
      '   Options:\n' +
      '     1. Set POLO_AI_RPC_TLS_CERT and POLO_AI_RPC_TLS_KEY to enable wss://\n' +
