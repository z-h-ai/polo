...
The public contract is `https://updates.polo.z-h-ai.com/electron/latest/release-contract.json`. Every resource below `/electron/latest/` (manifests, contract, scripts, and installer bytes) uses `Cache-Control: no-cache`, because the atomic `latest` pointer can change while its URLs remain fixed. Only files below a published strict-SemVer `/electron/releases/<version>/` directory use `Cache-Control: public, max-age=31536000, immutable`. The service accepts only GET and HEAD and returns 405 for mutation methods.
