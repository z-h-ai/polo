...
import { getSessionSafeAllowedToolNames } from '@polo-ai/session-tools-core';
...
class ModeManager {
  ...
  setPermissionMode(
    ...
    // Notify callbacks (for PoloAi internal sync)
...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.polo-ai/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .polo-ai entirely
  if (!normalizedTarget.includes('/.polo-ai/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__polo-ai-docs__')) {
      return { allowed: true };
    }
