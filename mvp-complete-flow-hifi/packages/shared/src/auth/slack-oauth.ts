...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = options.callbackUrl
    ?? `https://app.polo.z-h-ai.com/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects to the fixed loopback callback for this port.
    const redirectUri = `https://app.polo.z-h-ai.com/auth/slack/callback?port=${port}`;
