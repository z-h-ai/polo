...
**原型形态（v2 契约）**：`surface.html` 为纯产品表面，90 场景的交互全部静态声明为 1429 条 transitions，不含任何演示控制代码；`prototype.html` 为评审壳，集中承载页面索引、页面说明、本轮评审、故事播放与视口切换。两文件离线零网络请求，由 `validate_prototype.py` 以 manifest 精确校验（嵌入与文件 canonical 一致、边集一致、sha256 源绑定）。
...
要点：无活动直切、有活动先确认；全部终止后才提交切换；「取消切换」不撤销已完成的停止；目标加载失败可重试或留在原空间；无权目标从切换器移除并给原因。

### M03 首页与全部 Apps — 覆盖：11 屏 · 完整（调整关键页）
用户价值：常用 App 一步直达；其余按来源（企业/圈子/个人）可找、可查、可解释。
...
| **合计** | **90** | 90 场景 · 1429 条 transitions，全部自 `P-M01-INVITE-BROWSER` 可达（BFS 90/90） | M01—M11 全覆盖 |
...
- **「分支：」虚线按钮是评审入口**：无自然产品入口的失败/取消路径（登录验证码→冷启动、切换器→无权目标、重开恢复→追问恢复等）以虚线「分支：」按钮保证可达；它是评审分支入口，不是产品设计，产品化时须补真实入口（review.md §6 待裁决 9）。
- **评审 chrome 只在评审壳**：页面索引、页面说明、本轮评审、故事播放、视口切换、Back/Reset 全部在 `prototype.html`；`surface.html` 不含任何此类内容（校验器以字符串与结构双重断言）。
- **构建工具链**：`tools/extract_scenes.py`（自 v1 提取 90 场景 + 补分支边）→ `tools/build_surface.py`（组装 surface）→ `tools/build_manifest.py` / `tools/build_review.py` / `tools/sync_manifest.py` → `tools/smoke_review.py`（1429 边逐条点击核对）→ `tools/audit_viewports.py`（90×2 溢出/可达审计 + 36 张截图）。
