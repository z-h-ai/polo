...
版本：`POO-70 master-r9` · 日期：2026-09-17 · 父任务：POL-111 · 共享身份：POL-112 r11

> **唯一阅读与维护入口。** POO-70 卡片保留原始确认、变更记录和接受证据；本文件汇总客户端当前产品方案。`module-review.md`、`implementation-delta-review.md`、旧 `product-flow.html`、早期线框和 POO-71 高保真均是历史或派生评审材料。冲突时先按 POO-70 已接受决定与本文件处理；跨端共享冲突交 POL-116 收口。
>
...
本版把旧 Spec、集中功能方案和视觉收口整理成一份当前方案；补回 D-PC-08 的个人圈子/企业边界，按 POL-94 第二轮规则校正手动续费和充值恢复，并引用 POL-112 r11 的 ID-C01—10、D-ID-09—18。D-PC-01—07 的接受范围保持不变；本次编辑不把历史实现、任务 Done 或高保真页面提升为已发布/已验收。
...
| 场景 | 当前产品规则 | 用户看到的结果 |
| --- | --- | --- |
| 个人加入多个圈子 | 各圈子给同一个 personal ProductSpace 授予 App/Skill；各圈子的加入、续费、到期、退款和撤权互不影响 | 所有有效作品聚合进“我的空间”的目录与助手 Skills；“我的圈子”管理关系，空间选择器不会出现圈子 |
...
| 类型 | 当前结论 |
| --- | --- |
| 已确认并纳入客户端方案 | M01—M11、PC-F01—11、PC-N01—04、C-R01—08、J-PC-01—07；D-PC-01—08 |
...
| M02 空间切换 | 在“我的空间”和有效企业间安全切换 | 顶部空间选择器 | 保留；圈子不进入选择器；有活动先确认并全部停止 | 集成候选已有状态机；待验 App/助手/权限/计量整体切换 |
| M03 首页与全部 Apps | 打开助手、常用 App，浏览当前空间完整目录 | 首页；“全部 Apps” | 保留；助手固定，最多 5 个用户选择的常用 App；无公开市场 | 已有首页/Catalog；待补零常用、真空目录、失败和来源撤权状态 |
...
| M07 我的圈子 | 查看关系，经私域入口加入、订阅、续费、退出并取得作品 | 个人空间“我的圈子”；分享链接/二维码/邀请 | 保留完整产品规则；首发不建公开市场；企业无此模块 | 已有关系摘要和 Browser 部分能力；桌面详情/回跳/到期闭环待补 |
...
| PC-F03 加入、付费、续费、退出圈子 | S-010/011/014/015/016 | C4 仅非空关系摘要；A-INT 有 circle/join、circle/pay、memberships 页面/API | Browser 有局部实现；Desktop 详情/回跳未闭合，未做全程验收 | 已实现但流程不闭合；证据不足 | 调整：多圈子聚合到“我的空间”；私域入口不限邀请；按 A-PAY 完全手动续费，移出旧自动续费/宽限期；企业不订阅圈子；不建公开市场 |
...
- **退出/拒绝/空态**：立即退出须确认并撤销后续作品授权，关闭确认不退出；空关系引导用户使用已有分享链接、二维码或邀请，空作品解释尚未分发。链接失效/停止加入不建关系，需审批时不能先授予。
...
| 圈子，A-PAY FR-032—038 / A-DOC S-010—016 | 多圈子聚合到 personal ProductSpace；企业无圈子权益；私域入口可为分享/二维码/邀请；手动续费；内容与计算分账 | 后续支付规则明确替换旧自动续费/宽限期；客户端边界按 D-PC-08 补齐 | Browser 订阅服务、桌面详情/授权到期，不在本端重算期限；跨端作品归属差异交 POL-116 |
...
2. **跨端来源口径需由 POL-116 统一**：POL-68 早期创作者流程允许同一作品分发到多个圈子，而 POL-94 第二轮要求一个作品只属于一个圈子、多圈相似能力需分别形成作品。客户端不自行决定创作者数据模型；本端只保证多个圈子的最终有效权益聚合到“我的空间”，同名条目不因名称合并。
...
本轮没有发现需要用户立即新增选择才能整理本端正文的冲突。D-PC-08 是本轮用户直接明确的边界；一个作品能否跨多个圈子分发属于跨端数据模型冲突，交 POL-116 按 POL-68/POL-94 的接受范围统一，不由客户端静默改写其他端。
...
| 本卡 | 同 Client 仓库 `POO-70/docs/client-journey-policy-interview` | 整理基线 `6ba964f4df0ced8de0a4d7f0264246201deffdea`；保留既有 POO-71 原型 WIP，本轮只追加文档与一致性修订 |
...
| 来源 | 本次消费版本 | 用途 |
| --- | --- | --- |
| POO-70 | 整理输入 rev `78cf28a3e35dd97faf145e846c3a84fc065603513af3d600fe6f47e6739ef18b`；最终写回读回 rev `31d845c105702c93b1ee5ba20bdec427e44c80de753009e4ae256b3fb6fa1a83` | 本端已接受决定、D-PC-08、本说明唯一入口与视觉维护交接 |
| POL-112 | rev `c7545bbe507571abb840e75dd5891f5fb6b4c38cbc4884215389a1172e9e5545` | ID-C01—10、D-ID-09—18；共享账号、入口、会话与恢复，只引用不改写 |
| POL-116 | rev `6769300193f92410f3d8f830a5139bf44b31da5147e57c109330637c1a22cba8`；交接事件 `evt_3750ff41cd984b6e9dbc7bec817310b7` | 已收到跨端作品归属冲突、推荐及本端固定边界；仍待该卡收口 |
| POO-71 | 最终写回读回 rev `c8affcaee53b9e344bdebe09813403c7180af5ff5f7ccbebc45fcc99434e6d33`；状态 `done` | 已合并 master-r9 维护约定；原型是派生材料，不作为产品结论来源；旧运行未消费本修订 |
...
- 本轮共享边界：D-PC-08 来自 2026-09-17 用户直接要求，记录在 sources/user-decision-r9.md；它补齐圈子/企业边界，不自动把全部首发实现标成完成。
- 本轮基线、差异、任务读回与验证记录保存在 `sources/master-r9-change-log.md`；修订前 Spec 与 manifest 也已保留。
- 视觉：高保真只用于评审当前方案；本轮发现的明确规则遗漏直接修订为下一次走查材料。未获得 owner 视觉复看前保持“修改待复看”。
- 下一步：先完成 POO-71 的修订版视觉走查并记录行为/布局/视觉接受范围；产品进入开发前再以本文件和同版原型生成 Plan。POL-116 收拢跨端冲突与 policy 工程化。本次仍不编码或发布。
