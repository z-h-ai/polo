# POO-70 Polo 客户端 Spec v1（待整体接受）

日期：2026-09-15 · 父任务 POL-111 · 共享身份依据 POL-112 r11 · 本卡正文为产品结论权威，本地 spec.md 为同步副本，画布为派生产物。

Applied skills：ai-native-sdlc、spec-from-intent、product-flow-canvas；继承本卡 repo-capability-discovery 与 product-closure-review 盘点。当前阶段：Spec 审阅；未进入 Plan、开发或发布。
...
**本版仍待需求提出者整体接受 Spec 与变更范围。** 来源已确认的条款与本卡三项确认不因此失效。下述“保留/调整”是本次交付范围裁决清单；整体签核尚未发生。源码现状不等于此预期已经实现。
...
| C-R06 桌面权限 | 只在用户主动打开/选择材料时请求必要权限；首次权限说明、更新新增权限重新确认。拒绝或取消留原页面、不启动依赖该权限的动作；已在 OS 拒绝时给设置路径，用户授权后主动重试。撤销后下一次实际调用重新校验，不能仅凭旧 UI 开关继续读。此恢复交互为本版建议，沿用已有最小权限边界，不增加权限种类。 |
...
| 桌面权限，A-PAY PC-APP-01、POO-47 | 首次必要权限及新增权限确认、限制目录、不旁加载 | 边界可复用；C-R06 拒绝/设置/重试为本版恢复建议，OS 行为证据不足 | 实际文件选择/调用处校验，取消/撤销不越权；不新增权限类型 |
...
本轮没有发现需要立即增加一轮产品选择才能收口的冲突。整体接受仍未记录；以上关注项和延期范围须在整体审阅中可见。
...
- 本版整体流程及变更范围：**待产品 owner 接受**。C-R06 恢复交互等作者建议随本版审阅，不伪标为此前已确认。
- 视觉：继承未受影响 POO-41/POL-94 冻结，受影响系统工具入口后续按已接受范围修订；本轮低保真画布不构成新视觉冻结。
- 接受后下一步：进入 Plan，按真实集成基线对照原执行卡，列需要裁剪、复用、补充的实施与验收工作；POL-116 收拢跨端政策和影响范围。不需要等所有旧卡完成访谈；本次仍不编码或发布。
