...
- 独立复核时补齐真实 production wiring 闭环：明确失权且 denied 快照写盘失败后，再遇 `NETWORK_ERROR` 或 `TIMEOUT` 时，响应继续去除远程 URL 与 Bundle Release 交付字段；`RESOLVE_REMOTE_URL`、`INSTALL`、`START`、`RESTART` 均保持 `NOT_AUTHORIZED`，且不会物化 runtime manager。
...
- `cd apps/electron && bun run lint`：通过；0 errors、134 个仓库既有 warnings。
...
- `bun test ./apps/electron/src/main/handlers/__tests__/admin-local-app-session-ending.isolated.ts`：22 passed、193 assertions。
...
- `apps/electron/src/main/local-app-runtime/__tests__/manager.test.ts` 连续复跑 3 次：每次 30 passed、0 failed。
