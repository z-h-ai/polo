...
describe('packaged server resolution', () => {
  ...
  it('starts an isolated runtime despite an Electron-owned global lock', async () => {
    ...
    expect(existsSync(globalLock)).toBe(true)
    expect(existsSync(join(server.runtimeDir, '.server.lock'))).toBe(true)
    expect(client.isConnected).toBe(true)

    client.destroy()
    await server.stop()
    expect(existsSync(server.runtimeDir)).toBe(false)
  }, 120_000)
})
