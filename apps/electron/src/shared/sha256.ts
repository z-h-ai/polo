...
/**
 * Incremental SHA-256 following NIST FIPS 180-4. It lives in the shared
 * layer so both the renderer (chunked upload hashing without Node crypto)
 * and Main/renderer partition naming share one audited implementation.
 *
...
/** Convenience helper: SHA-256 of a UTF-8 string, as lowercase hex. */
export function sha256HexFromUtf8(input: string): string {
  const hasher = new IncrementalSha256()
  hasher.update(utf8Bytes(input))
