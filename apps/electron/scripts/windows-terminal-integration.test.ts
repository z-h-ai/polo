...
describe('Windows terminal integration packaging', () => {
  ...
  it('persists PATH ownership and only removes entries owned by Polo', () => {
    ...
    expect(integrationScript).toContain('[IO.File]::ReadAllText($Path, [Text.Encoding]::UTF8)')
    ...
    expect(integrationScript).toContain('Uninstall-Transactional ([bool]$UserPathFile) ([bool]$RequireOwnershipState)')
    ...
    expect(integrationScript).toContain('required ownership state is absent')
