...
describe('Windows terminal integration packaging', () => {
  ...
  it('persists PATH ownership and only removes entries owned by Polo', () => {
    ...
    expect(integrationScript).toContain('schemaVersion = 4')
    ...
    expect(integrationScript).toContain('Uninstall-Transactional ([bool]$UserPathFile) ([bool]$RequireInstallBinding)')
    ...
    expect(integrationScript).toContain('ownership state from a different installation directory')
    expect(integrationScript).toContain('installation-bound ownership state is absent')
