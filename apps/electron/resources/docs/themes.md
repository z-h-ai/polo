...
This guide explains how to customize the visual theme of Polo AI.

## Overview

Polo AI uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.polo-ai/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.polo-ai/theme.json` - Override specific colors (app-level)
...
```
~/.polo-ai/workspaces/{id}/config.json
```
...
Create `~/.polo-ai/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.polo-ai/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.polo-ai/themes/{name}.json`
...
1. Create `~/.polo-ai/theme.json` for overrides or `~/.polo-ai/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.polo-ai/theme.json` for overrides, `~/.polo-ai/themes/` for presets)
