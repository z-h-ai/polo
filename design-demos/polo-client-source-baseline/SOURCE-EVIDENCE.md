...
| Reset confirmation dialog | `src/source/ResetDialog.jsx` | `ResetConfirmationDialog.tsx`, `dialog.tsx`, `button.tsx`, `input.tsx`, `zh-Hans.json` (`dialog.reset.*`) | direct source translation is integrated on the popover-styled dialog skeleton; the source's per-open random arithmetic challenge is retained, and a real Renderer screenshot is still required before it can be classified source-faithful |
| Browser first-load empty state | `src/source/BrowserEmptyState.jsx` | `browser-empty-state.tsx`, `components/browser/empty-state-prompts.ts`, `packages/ui/src/components/ui/BrowserEmptyStateCard.tsx`, `zh-Hans.json` (`browser.*`) | source translation is integrated and compared against `reference-playground-browser-empty-1440x900.png` (zh-Hans title/description/safety hint, English sample prompts as in source); the BrowserView/native browser process remains explicitly approximate |
| Keyboard shortcuts dialog | `src/source/KeyboardShortcutsDialog.jsx` | `KeyboardShortcutsDialog.tsx`, `renderer/actions/definitions.ts`, `dialog.tsx`, `zh-Hans.json` (`shortcuts.action.*`, `shortcuts.category.*`) | direct source translation is integrated for the macOS fixture (registry + component-specific sections); a real Renderer screenshot is still required before it can be classified source-faithful |
