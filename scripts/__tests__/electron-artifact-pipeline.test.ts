...
describe('Electron final artifact validation pipeline', () => {
  ...
  it('validates the installed NSIS container and real previous-to-current lifecycle', () => {
    ...
    expect(validator).toContain('-RawArgumentLine "/S _?=$installDir"')
    expect(validator).toContain('$startInfo.Arguments = $RawArgumentLine')
    ...
    expect(nsis).toContain('WriteRegStr HKCU "${INSTALL_REGISTRY_KEY}" "PoloTerminalBinDir" "$R1"')
    expect(nsis).toContain('ReadRegStr $R1 HKCU "${INSTALL_REGISTRY_KEY}" "PoloTerminalBinDir"')
    expect(nsis).not.toContain('polo-terminal-integration.ini')
    ...
    expect(nsis).not.toContain('!macro customUnInit')
  ...
  it('defines a reusable blocking four-platform release build with an unsigned manual Windows installer', () => {
    ...
    expect(workflow).toContain('lookup github.com: no such host')
    ...
    expect(workflow).toContain('EBUSY: resource busy or locked')
