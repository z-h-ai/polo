...
describe('Electron release workflow boundaries', () => {
  ...
  it('uses an observable fail-closed compensation state machine before publishing the Draft Release', () => {
    ...
    expect(rollbackAction).toContain('publish-electron-release.ts assert-not-latest')
    ...
    expect(release).toContain("if: steps.github-publish.outcome != 'success' || steps.github-publish-state.outcome != 'success'")
    expect(release).toContain('gh api --method PATCH')
