...
allowed-tools: [spawn_agent, Read, Write, Bash, AskUserQuestion, Grep, Glob]
user-invocable: true
---
...
1. **Read** the phase file from `.agents/skills/hermes-coding/` (e.g., `.agents/skills/hermes-coding/phase-1-clarify.md`)
2. **Spawn** a fresh Agent with the phase content as prompt:

```
Tool: spawn_agent
Parameters:
  task_name: "Execute {phase-name} phase"
  fork_turns: "none"
  message: |
    ...
    - Bootstrap: source .agents/skills/hermes-coding/bootstrap-cli.sh
