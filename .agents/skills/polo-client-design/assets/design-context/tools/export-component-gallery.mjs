...
const repositoryRoot = process.env.POLO_REPO_ROOT || execFileSync('git', ['rev-parse', '--show-toplevel'], { cwd: root, encoding: 'utf8' }).trim()
const outDir = resolve(root, 'components')
const buildDir = resolve(root, 'dist-gallery')
const viteBin = process.env.VITE_BIN || resolve(repositoryRoot, 'node_modules/.bin/vite')
const viteModule = process.env.POLO_VITE_MODULE || resolve(repositoryRoot, 'node_modules/vite/dist/node/index.js')
...
const poloIconData = 'data:image/png;base64,' + readFileSync(resolve(root, 'assets/renderer/polo-app-icon.png')).toString('base64')
const entrySource = `
...
const poloIconSrc = ${JSON.stringify(poloIconData)}
const registry = {
...
const reactPluginModule = await import(pathToFileURL(resolve(repositoryRoot, 'node_modules/@vitejs/plugin-react/dist/index.js')).href)
...
await vite.build({
  ...
  resolve: {
    alias: {
      ...
      'react-dom/server': resolve(repositoryRoot, 'node_modules/react-dom/server.node.js'),
      'lucide-react': resolve(repositoryRoot, 'node_modules/lucide-react'),
    },
  },
  build: {
    outDir: buildDir,
